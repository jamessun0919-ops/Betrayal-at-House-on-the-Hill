...
## category 說明
weapon武器，選項：手持／給予／遺留，應標明以哪種能力襲擊，是否影響擲骰
gear裝備，選項：配戴／給予／遺留，配戴後產生效果。配戴中選項切換為取下
consumable消耗品，選項：使用／給予／遺留，消耗品使用一次後消失，除非text說明使用後返回牌堆
Reusable重複使用，選項：使用／給予／遺留，可重複使用的物品。
decoration裝飾品，選項：給予／遺留，無用的道具，單純設計來平衡抽取道具量，或塞彩蛋
imprint銘印，選項：查看，omen限定的效果，不可被偷竊、給予、遺留，可被其他物品或事件導致消除。