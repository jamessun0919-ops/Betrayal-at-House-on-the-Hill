const { placeRoomAt, coordKey } = require('./boardGenerator');
...
// into a basement room at the same (x, y). No physical damage is applied
// here (M3 damage-distribution system doesn't exist yet -- this is a known,
// deliberate gap, not an oversight).
function dropToBasement(gameState, player, currentRoom) {
  let basementRoom = gameState.board.basement.get(coordKey(currentRoom.x, currentRoom.y));
  if (!basementRoom) {
