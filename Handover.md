...
- **M2 拆成子計畫依序執行**（M2a/M2b/M2c，是 M2 內部分階段；M2c 又再細分 M2c-1~M2c-5，見下方「下一步行動」）：
  ...
  - **M2c：卡牌牌庫＋效果解析器**：
    ...
    - **M2c-3（36 張卡片＋房間操作類 effects 內容）—— 尚未開始，是下一步**（範圍已擴充，見下方）
    - **M2c-4（道具/操作動作接線）—— 已規劃，尚未寫 spec**
    - **M2c-5（邪祟考驗機制）—— 已規劃，尚未寫 spec**
...
- **M2c-4 規劃時務必套用同一套模式**：「道具」「操作」這兩個 `selectAction` 動作接上真實邏輯時，如果會產生需要玩家選擇的效果，必須用上面的 `pendingChoice`/`EFFECT_CHOICE_IN_PROGRESS` 機制，不能重新設計一套
...
無設計層面阻塞。M2c-1、M2c-2 已完整合併進 `main`（268/268 測試通過）。開發者要求 M2c-3 完成後要從頭手動跑一遍完整流程（見下），因此對照這個手動測試流程重新盤點了 M2 收尾前還缺什麼，發現原本規劃的 M2c-3（只有卡片內容）不足以支撐完整測試——已跟開發者確認擴充範圍與新增 M2c-4/M2c-5/M2d 三個里程碑（見下）。這些新里程碑都**還沒寫 spec 文件**，是下一階段的工作。

## 下一步行動 (Next Steps)
1. 讀取本 Handover；worklog 讀 2026-08-05（今日）範圍即可
2. **M2c-3（卡片＋房間操作 effects 內容，範圍已擴充）**：36 張事件/道具卡的 `effects` 內容 **加上**房間「操作」類 effects 內容（例如 `data/rooms/rooms.json` 的「保險庫」已有文字描述但 `effects:[]` 是空的）**加上** `omen-cards.json` 補 `needsCustomLogic` 欄位（目前完全沒有這個欄位，跟 event/item 卡的 schema 不一致）——agent 依 `card-mechanics-reference.md` 草擬 JSON，開發者審核修正
3. **M2c-4（道具/操作動作接線）**：把 `turnFlow.js` 的 `selectAction` 的 `item`/`room_action` 兩種 `actionType` 從殼子接上真實邏輯——玩家主動觸發（不是像抽卡一樣自動），沿用 M2c-1/M2c-2 已建好的 `effectResolver.resolveEffects`＋`effectResolverManager`/`pendingChoice`/`EFFECT_CHOICE_IN_PROGRESS` 機制。**本次明確不做**兩層 20 秒計時提問 UI（`turn-flow-and-action-points.md` 8/1 已核准的設計），改用跟現有 `game:move` 一致的簡單直接事件模式（點選項、送出、直接回結果，沒有倒數/逾時放棄回合）——**20 秒計時 UI 延後到 M2 完整測試流程全部跑完之後再補**，記得到時候要回頭補上，不要漏掉
4. **M2c-5（邪祟考驗機制）**：目前 `gameState.omenCount` 欄位存在但從未被遞增，也完全沒有「什麼時候觸發邪祟」的規則實作。**開發者已定案的簡化版規則**（不是實體遊戲「同名預兆抽第二張」的規則，因為我們的 13 張預兆互不重複，那條規則不適用）：**每次抽到一張預兆卡，`omenCount` 遞增後，骰數量等於當前 `omenCount` 的骰子（沿用 0/0/1/1/2/2 骰面），總和 >5 就觸發邪祟**（`gameState.hauntStarted = true`，這個欄位從 M2a 就存在）。觸發後只需要正確廣播「邪祟降臨」讓測試流程知道要停在這裡即可，不需要做任何 M3 的戰鬥/陣營內容
...
6. **執行順序已跟開發者確認**：M2c-3 → M2c-4 → M2c-5 → M2d，依序寫 spec/計畫並執行，不要打亂
7. **M2c-3/4/5/M2d 全部完成後，開發者要手動從頭跑一次完整流程**：建房→加入→鎖門（目前是選角開始時隱含鎖門，不是獨立按鈕，已跟開發者確認這個理解一致）→隨機選角→開始遊戲→（迴圈）選擇行動/開門/移動/觸發房間效果/觸發卡片效果/改變狀態/結束回合換人，直到邪祟考驗觸發邪祟為止。邪祟觸發後的戰鬥內容是 M3，這次測試不涵蓋
...
- **邪祟觸發規則（M2c-5，開發者已定案）**：每抽一張預兆卡，`omenCount` 遞增，骰 `omenCount` 顆骰子（0/0/1/1/2/2 面），總和 >5 觸發 `hauntStarted=true`——不是實體遊戲的「同名預兆抽第二張」規則
